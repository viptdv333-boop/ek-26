...
const WALLPAPER_PRESETS = [
  { id: 'default', labelKey: 'wallpaper.default', gradient: 'radial-gradient(ellipse at 50% 0%, rgba(220,38,38,0.1) 0%, transparent 45%), linear-gradient(180deg, #0f0f0f 0%, #141010 50%, #0f0f0f 100%)' },
  // Dark (3)
  { id: 'midnight-aurora', labelKey: 'wallpaper.midnightAurora', gradient: 'radial-gradient(ellipse at 20% 80%, rgba(56,189,248,0.1) 0%, transparent 50%), radial-gradient(ellipse at 80% 20%, rgba(139,92,246,0.12) 0%, transparent 50%), linear-gradient(135deg, #0a0a1a 0%, #0f172a 40%, #1e1b4b 100%)' },
  { id: 'obsidian-glow', labelKey: 'wallpaper.obsidianGlow', gradient: 'radial-gradient(ellipse at 50% 0%, rgba(220,38,38,0.14) 0%, transparent 50%), radial-gradient(ellipse at 80% 100%, rgba(251,146,60,0.08) 0%, transparent 50%), linear-gradient(180deg, #0f0f0f 0%, #1a0a0a 50%, #0f0f0f 100%)' },
  { id: 'deep-ocean', labelKey: 'wallpaper.deepOcean', gradient: 'radial-gradient(ellipse at 30% 70%, rgba(6,182,212,0.12) 0%, transparent 50%), radial-gradient(ellipse at 70% 30%, rgba(59,130,246,0.1) 0%, transparent 50%), linear-gradient(160deg, #0a0f1a 0%, #0c1929 40%, #0a1628 100%)' },
  // Light (3)
  { id: 'arctic-frost', labelKey: 'wallpaper.arcticFrost', gradient: 'radial-gradient(ellipse at 30% 20%, rgba(147,197,253,0.35) 0%, transparent 50%), radial-gradient(ellipse at 70% 80%, rgba(196,181,253,0.25) 0%, transparent 50%), linear-gradient(150deg, #f0f4ff 0%, #f5f3ff 50%, #eff6ff 100%)' },
  { id: 'rose-quartz', labelKey: 'wallpaper.roseQuartz', gradient: 'radial-gradient(ellipse at 25% 75%, rgba(251,207,232,0.4) 0%, transparent 50%), radial-gradient(ellipse at 75% 25%, rgba(254,205,211,0.3) 0%, transparent 50%), linear-gradient(135deg, #fff5f7 0%, #fef2f2 50%, #fff1f2 100%)' },
  { id: 'golden-hour', labelKey: 'wallpaper.goldenHour', gradient: 'radial-gradient(ellipse at 30% 60%, rgba(253,224,71,0.2) 0%, transparent 50%), radial-gradient(ellipse at 70% 30%, rgba(251,146,60,0.15) 0%, transparent 50%), linear-gradient(135deg, #fffbeb 0%, #fef3c7 30%, #fff7ed 100%)' },
];

type Section = 'language' | 'appearance' | 'notifications' | 'contacts' | 'devices' | 'widget' | 'games' | 'faq' | 'about';
      ...
      return (
        <div className="fixed inset-0 z-[70] bg-dark-900 flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 bg-dark-800 border-b border-dark-600">
            <button onClick={() => setActiveGame(null)} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
              </svg>
              <span className="text-sm">Назад</span>
            </button>
            <span className="text-sm font-medium text-white">{gamesList.find(g => g.url === activeGame)?.name}</span>
            <button onClick={() => setActiveGame(null)} className="text-gray-400 hover:text-white transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          </div>
          <iframe src={activeGame} className="flex-1 w-full border-0" allow="autoplay" />
        </div>
      );
    }
      ...
      <div className="grid grid-cols-2 gap-3">
        {gamesList.map((game) => (
          <button
            ...
            className="flex flex-col items-center gap-2 p-4 bg-dark-600 hover:bg-dark-500 rounded-xl transition-colors group"
          >
        ...
        <div className="flex flex-col items-center justify-center gap-2 p-4 bg-dark-600/30 rounded-xl border border-dashed border-dark-500 text-gray-600">
